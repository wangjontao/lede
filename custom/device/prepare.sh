...
make defconfig
grep -q '^CONFIG_TARGET_mediatek_filogic_DEVICE_jdcloud_re-cs-05=y$' .config
for p in kmod-mt7915e kmod-mt7986-firmware luci-app-passwall luci-app-passwall2 luci-app-homeproxy luci-app-openclash luci-app-store quickstart luci-app-quickstart luci-theme-argon luci-app-ttyd luci-app-nps npc luci-app-ddns luci-app-nlbwmon luci-app-wol luci-app-vlmcsd; do grep -q "^CONFIG_PACKAGE_${p}=y$" .config || { echo "Required package missing: $p"; exit 1; }; done
